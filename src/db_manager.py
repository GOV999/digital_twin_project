...
def insert_meter_details(meter_id: str, meter_no: str, location: str = None):
    ...
    Uses ON CONFLICT DO UPDATE to handle upsert logic.
    """
    conn = None
    try:
        conn = get_db_conn()
        with conn.cursor() as cur:
            cur.execute(
                ...
                RETURNING meter_id; -- Return the meter_id of the affected row
                """,
                (meter_id, meter_no, location, meter_id) # Pass meter_id again for COALESCE
            )
            inserted_id = cur.fetchone()[0] # Fetch the returned meter_id
            conn.commit()
