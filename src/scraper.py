...
def extract_all_data_from_paginated_table(driver, full_fetch=False):
    all_raw_data = []
    ...
    while len(all_raw_data) < MAX_READINGS:
        ...
        remaining = MAX_READINGS - len(all_raw_data)
        all_raw_data.extend(current_page_data[:remaining])

        if len(all_raw_data) >= MAX_READINGS or not full_fetch:
            break
        ...
        try:
            ...
            logger.warning(f"Pagination failed or ended: {e}")
            break
    ...
    try:
        ...
        while True:
            try:
                ...
                raw_extracted_readings =extract_all_data_from_paginated_table(driver, full_fetch=first_run)
                first_run = False
